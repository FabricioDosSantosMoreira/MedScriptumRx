    ...
    setSelectionMap((prev) => {
      ...
      if (mode === 'includeAll') {
        ...
        if (cur.selectedProductIDs.size === 0 && prescriptions.length) {
          ...
          if (p) p.products.forEach((prod) => cur.selectedProductIDs.add(prod.internalSystemID ?? prod.uniqueID));
        }
        cur.expanded = true;
      }
  ...
  function toggleProductInPrescription(
    ...
    setSelectionMap((prev) => {
      ...
      if (!copy[prescriptionID]) {
        copy[prescriptionID] = {
          includeAll: false,
          selectedProductIDs: new Set([productPDVID]),
          expanded: true,
        };
      } else {
        const cur = copy[prescriptionID];
        const setCopy = new Set(cur.selectedProductIDs);
        if (setCopy.has(productPDVID)) setCopy.delete(productPDVID);
        else setCopy.add(productPDVID);
    ...
    for (const prescID of selectedPrescriptionIDs) {
      ...
      const productsToIterate = sel.includeAll
        ...
        : presc.products.filter((prod) =>
            sel.selectedProductIDs.has(prod.internalSystemID ?? prod.uniqueID)
          );

      console.log(productsToIterate);

      productsToIterate.forEach((prod) => {
        const key = prod.internalSystemID ?? prod.uniqueID;
        if (!aggMap[key]) {
          aggMap[key] = {
                      ...
                      {selection && selection.expanded && (
                        <Tr>
                          <Td colSpan={5} style={{ padding: 8 }}>
                            <CollapsibleProducts>
                              {p.products.map((prod) => {
                                const prodKey = prod.internalSystemID ?? prod.uniqueID;
                                const checked =
                ...
                <tbody>
                  {aggregatedItems.map((it, idx) => (
                    <tr key={it.internalSystemID + idx}>
                      <ItemsTd></ItemsTd>
                      <ItemsTd $maxWidth='10%'>{it.internalSystemID}</ItemsTd>
                      <ItemsTd>{it.name + ' - ' + it.totalQty + 'x' + it.presentation}</ItemsTd>
                      <ItemsTd>{formatCurrency(it.value)}</ItemsTd>
                      <ItemsTd>{formatCurrency(it.value * it.totalQty)}</ItemsTd>
                      <ItemsTd $maxWidth='5%'>{it.totalQty}</ItemsTd>
                    </tr>
                  ))}
                </tbody>
              </ItemsTable>
